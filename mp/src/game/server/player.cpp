...
void CBasePlayer::CreateHandModel(int index, int iOtherVm)
{
	...
	if (vm)
	{
		...
		vm->SetIndex(index);
		DispatchSpawn(vm);
		vm->FollowEntity(GetViewModel(iOtherVm));
		m_hViewModel.Set(index, vm);
	}
}
...
void CBloodSplat::Think( void )
{
	...
	if (pPlayer && g_Language.GetInt() != LANGUAGE_GERMAN && UTIL_ShouldShowBlood(pPlayer->BloodColor()))
	{
		Vector forward;
		AngleVectors( GetAbsAngles(), &forward );
		UTIL_TraceLine ( GetAbsOrigin(), GetAbsOrigin() + forward * 128, 
...
#ifdef PONEDM
		GiveAmmo(255,	"Gatling");
		GiveAmmo(255,	"Railgun");
#endif
#ifdef HL2_EPISODIC
		GiveAmmo( 5,	"Hopwire" );
#endif		
