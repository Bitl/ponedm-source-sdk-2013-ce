...
ConVar sv_ponedm_bot_fillserver("sv_ponedm_bot_fillserver", "0", FCVAR_GAMEDLL | FCVAR_NOTIFY);
...
static HL2MPViewVectors g_HL2MPViewVectors(
	Vector( 0, 0, 48 ),       //VEC_VIEW (m_vView) 
							  
	Vector(-16, -16, 0 ),	  //VEC_HULL_MIN (m_vHullMin)
	Vector( 16,  16,  56 ),	  //VEC_HULL_MAX (m_vHullMax)
	...
	Vector( 0, 0, 18 ),		  //VEC_DEAD_VIEWHEIGHT (m_vDeadViewHeight)
...
CHL2MPRules::CHL2MPRules()
{
	...
	m_tmNextZombieModeThink = gpGlobals->curtime + 25.0;
...
void CHL2MPRules::PlayerSpawn(CBasePlayer* pPlayer)
{
	...
	if (sv_ponedm_bot_fillserver.GetBool() && !pPlayer->IsBot())
	{
		int imaxClients = gpGlobals->maxClients - 1;
		SpawnPoneDMBots(imaxClients);
	}
#endif
}
...
//	float flTimeLimit = mp_timelimit.GetFloat() * 60;
	...
	if ((!IsTeamplay() && (sv_ponedm_gamemode.GetInt() == 3)) && gpGlobals->curtime > m_tmNextZombieModeThink)
	{
		...
		{
			bool infection = true;
			DevMsg("%s: Initalizing bot infection.\n", GetGameDescription());

			for (int i = 1; i <= gpGlobals->maxClients; i++)
			{
				CBasePlayer* pPlayer = UTIL_PlayerByIndex(i);

				if (pPlayer && !pPlayer->IsFakeClient() && (pPlayer->GetTeamNumber() == TEAM_ZOMBIES))
				{
					DevMsg("%s: No need to start bot infection. Players will deal with them.\n", GetGameDescription());
					infection = false;
					break;
				}
			}
...
CAmmoDef *GetAmmoDef()
{
	...
	if ( !bInitted )
	{
		...
		//def.AddAmmoType("AR2",				DMG_BULLET,					TRACER_LINE_AND_WHIZ,	0,			0,			999,			BULLET_IMPULSE(200, 1225),	0 );
		...
		def.AddAmmoType("RPG_Round",		DMG_BURN,					TRACER_NONE,			0,			0,			5,			0,							0 );
