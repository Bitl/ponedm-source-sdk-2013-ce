...
#else
	PrecacheScriptSound("Pony.RunFootstepLeft");
	PrecacheScriptSound("Pony.RunFootstepRight");
#endif
}
...
void CHL2MP_Player::PlayStepSound( Vector &vecOrigin, surfacedata_t *psurface, float fvol, bool force )
{
	...
	if (m_Local.m_nStepside)
	{
		Q_snprintf(szStepSound, sizeof(szStepSound), "Pony.RunFootstepLeft");
	}
	else
	{
		Q_snprintf(szStepSound, sizeof(szStepSound), "Pony.RunFootstepRight");
	}
#endif
